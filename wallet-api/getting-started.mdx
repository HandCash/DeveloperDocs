...
<Warning>
The public npm package **`@handcash/sdk` does not include Wallet API clients** — there is no `WalletService` export. Use **REST** against `https://cloud.handcash.io` or generate a client from the OpenAPI spec (tag: **Wallet API**).
</Warning>
...
Do **not** use the Connect quick start (`npm install @handcash/sdk`) for Wallet API integration — that package only covers Connect, Items, Users, Minter, and Payment Requests.
