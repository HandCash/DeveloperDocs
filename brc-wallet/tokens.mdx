...
The canonical encoding is **BRC-162 binary** value locks. Legacy BRC-161 JSON inscriptions (`application/bsv-20`) and BRC-175 `1sat-ft` exist for reading older holdings; the wallet refuses to emit `1sat-ft` on a send or burn, so do not build new products on them.
...
|--------|--------------|
| `p bsv21 all` | none |
| `p bsv21 id` | `bsv21:<tokenId>` (legacy `id:<tokenId>` also matches) |
...
Sum the tips yourself; there is no per-token balance method. A third-party grant is an **allowlist of token ids**, so asking for `all` later never widens it — the user must approve each new token. Legacy `1sat-ft` rows are never returned to apps, and `listOutputs({ basket: 'bsv21' })` returns `400 USE_PBSV21_SCOPE`.
...
Failures are named rather than silent: insufficient units, a zero-amount authority spend, an attempt to emit `application/1sat-ft+json`, an invalid recipient, missing source transactions, conservation mismatches, and `BRC-176 prove failed` all surface as errors instead of a downgraded transaction.
...
The named output must be one satoshi, must pay the wallet, and must decode as a BRC-162 value lock (or a legacy JSON `transfer` matching the same token id and amount). Anything else is refused with a named reason such as an ambiguous token output or no token tip paying this wallet.
...
| Minting or sending `1sat-ft` / new JSON inscriptions | Refused on the send path |
